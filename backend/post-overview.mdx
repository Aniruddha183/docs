...
flowchart TD
  ...
  G -->|unused keys| H[Delete unused S3 objects & cleanup DB]
  G --> I[Create/Update AdminReview(stage=reviewer)]
  I --> J[Reviewer reviews]
  J -->|save draft comment| I
  J -->|request_changes| K[Return to Author with comments]
  J -->|send_to_approver| L[Approver stage; reviewer lock=true]
  L --> M[Approver reviews; may edit minor fields]
  M -->|approve| N[Publish Post -> set status=published; version++]
  N --> O[Notify subscribers; index for search; enable public view]
  M -->|deny| P[Set status=denied; notify author with reason]
  N --> Q[Users view, like, comment, report]
  Q --> R[Analytics & moderation flows]
```
