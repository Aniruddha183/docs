---
title: 'Newsletter Service — Zoho Groups, Templates & Email Pipeline'
...
3. **Send newsletters** automatically through an SNS → SQS → Lambda fan-out.
...
<AccordionGroup>
  ...
  <Accordion title="🔑 Initial bootstrap">
    <Steps>
      <Step title="Purpose">
...
(async () => {
  ...
  /* 3️⃣ persist (replace any old row) */
...
<Steps> 
    ...
    * If <code>expiresAt – now&nbsp;&lt;&nbsp;120 s</code> it calls
...
<AccordionGroup>
    ...
    ## Body Parameters
        ...
        |--------------------|-----------|----------|-------------------------------------------------------------|
        | `name`             | string    | ✅       | Friendly display name for the group                         |
        | `emailId`          | string    | ✅       | Unique group email address that Zoho will assign            |
        | `description`      | string    | ✅        | Longer description shown in the admin UI                    |
        | `createdBy`        | string    | ✅       | User ID of the admin creating the group (stored for auditing)|
    ...
    ## Body Parameters
        | Field               | Type      | Required | Description                                                 |
        |--------------------|-----------|----------|-------------------------------------------------------------|
        | `mailGroupMemberList`             | object[]    | ✅       | Array of objects, each with a `memberEmailId` key containing the new member’s email address.                         |
        | `membersNames`                    | string[]    | ✅       | 	Parallel array of display names. If omitted, each name defaults to the part of the email before “@”.           |
    ...
    | Field                 | Type      | Required | Description                                                                                           |
    |-----------------------|-----------|----------|-------------------------------------------------------------------------------------------------------|
    | `mailGroupMemberList` | object[]  | ✅       | Array of objects, each with a `memberEmailId` key specifying the member email address to be removed.  |
    ...
    #### 400 Unauthorized
    ```bash
        { "error": "mailGroupMemberList must be a non‑empty array" }
    ...
    ```bash
        ...
        | Field       | Type      | Required | Description                                                        |
        |-------------|-----------|----------|--------------------------------------------------------------------|
        | `groupNames`| string[]  | ✅       | Array of **group names** to delete. Must contain at least one item.|
        ...
        #### 400 Bad Request
        ```bash
                { "error": "groupNames must be a non‑empty array" }
        ```
        ...
        |--------------|--------|----------|-----------------------------------------------------------|
        | `identifier` | string | ✅       | Member’s **email** or **display name** (case‑insensitive) |
    ...
    ---
        ...
        | Param        | Type   | Required | Description                                                                                 |
        |--------------|--------|----------|---------------------------------------------------------------------------------------------|
        | `groupName`  | string | ✅       | Exact newsletter group name. *At least one of `groupName` or `groupEmail` must be provided. |
        | `groupEmail` | string | ✅       | Newsletter group email address.                                                             |
    ...
    ---
        ...
        |---------|--------|----------|---------|--------------------------------------------------------------------------------------------------|
        | `search` (Optional)| string | ✅       |  —      | Case‑insensitive filter applied to member name, member email, group name, or group email fields. |
        ...
        | 400    | Query parameters invalid (e.g., non‑numeric page). |
        | 500    | Internal server / database error.                  |
        ---
        #### 200 OK
        ```bash
    ...
    ---
       ...
       ```bash
        ...
        | Param    | Type   | Required | Default | Description                                                                                                                         |
        |----------|--------|----------|---------|-------------------------------------------------------------------------------------------------------------------------------------|
        | `search` | string | ✅       |  —      | Case‑insensitive text filter applied to member name/email and group name/email.                                                    |
        | `groups` | string | ✅       |  —      | Comma‑separated list of **group names**; only members from these groups will be returned.                                           |
        | `start`  | date   | ✅       |  —      | ISO‑8601 start date (`YYYY‑MM‑DD`). Filters members whose `dateJoined` is **on/after** this date.                                  |
        | `end`    | date   | ✅       |  —      | ISO‑8601 end date (`YYYY‑MM‑DD`). Filters members whose `dateJoined` is **on/before** this date.                                    |
        ...
        ```bash
            ```bash
            { "error": "start and end must be valid ISO‑8601 dates" }
...
<AccordionGroup> 
    ---
       ...
       ```bash
        ...
        | Field               | Type      | Required | Description                                                                                                  |
        |---------------------|-----------|----------|--------------------------------------------------------------------------------------------------------------|
        | `name`              | string    | ✅       | Unique template name (also used as SES `TemplateName`).                                                      |
        | `contentHTML`       | string    | ✅       | Full HTML body of the email.                                                                                 |
        | `subject`           | string    | ✅       | Email subject line (defaults to **"Default Subject"** if omitted).                                          |
        | `createdBy`         | string    | ✅       | User ID of the admin creating the template.                                                                  |
        | `category`          | string    | ✅       | Business‑level category (e.g., *Onboarding*, *Promotions*).                                                  |
        | `title`             | string    | ✅       | Human‑friendly title shown in the admin UI.                                                                  |
        | `type`              | string    | ✅       | Template type (e.g., `HTML`, `Text`).                                                                        |
        | `groupOfUserSend`   | string    | ✅       | Name of the newsletter group the template will target.                                                       |
        | `scheduledDate`     | date      | ✅       | **IST** date (`YYYY‑MM‑DD`) for scheduled send.                                                              |
        | `scheduledTime`     | time      | ✅       | **IST** time (`HH:mm`) for scheduled send.                                                                   |
        | `state`             | string    | ✅       | Initial state: `draft`, `scheduled`, etc.                                                                    |
        | `userGroup`         | string    | ✅       | Custom tag grouping (for analytics/audience segmentation).                                                   |
        ...
        ```bash
            {
                ...
                "template": {
                    ...
                    "state"           : "scheduled",
                    "newsLetterID"    : "TMP‑000142"                  // generatedId middleware
                }
            }
            ```
        #### 400 Bad Request
        ...
        #### 409 Conflict
        ```bash
            { "error": "Template 'Welcome‑2025‑05' already exists in SES." }
        ```
        ...
        ## Request
       ```bash
        curl "https://{your-host}/api/fitnearn/web/admin/getTemplate?templateName=Welcome‑2025‑05" \
        ```

        ## Query Parameterrs
        | Field          | Type   | Required | Description                                 |
        |----------------|--------|----------|---------------------------------------------|
        | `templateName` | string | ✅       | Exact name of the template to be retrieved. |
        ...
        #### 404 Not Found
        ```bash
            { "error": "Template 'Welcome‑2025‑05' not found in DB" }
        ```
        ...
        ## Request
       ```bash
            curl "https://{your-host}/api/fitnearn/web/admin/deleteTemplate/Welcome‑2025‑05" \
            -X DELETE \
        ```
        ...
        | Field | Type   | Required | Description                                    |
        |-------|--------|----------|------------------------------------------------|
        | `name`| string | ✅       | Exact template name to delete.                 |
        ...
        ```bash
            {
                "success"      : true,
                "message"      : "Template 'Welcome‑2025‑05' deleted from SES and MongoDB.",
                "removedMongoDoc": {
        ...
        ```bash
            {
                "error": "Template 'Welcome‑2025‑05' not found in MongoDB (SES deletion attempted)."
            }
        ```
                ...
                | `search`| string | ✅       | Filters by substring match on template `title` or `name`.    |
                ...
                ## Request
                ```bash
                    curl "https://{your-host}/api/fitnearn/web/admin/editTemplate/Welcome‑2025‑05" \
                    -X PUT \
                    -H "Content-Type: application/json" \
                    -d '{
                ...
                | Field | Type   | Required | Description                                   |
                |-------|--------|----------|-----------------------------------------------|
                | `name`| string | ✅       | Name of the template to update (SES key).     |

                ## Body parameters
                | Field               | Type   | Required | Description                                                          |
                |---------------------|--------|----------|----------------------------------------------------------------------|
                | `subject`           | string | ✅       | New subject line. If omitted, subject remains unchanged.             |
                | `contentHTML`       | string | ✅       | New HTML content. If omitted, HTML remains unchanged.                |
                | `category`          | string | ✅       | Metadata category (e.g., Onboarding, Notification).                  |
                | `title`             | string | ✅       | Display title for the template card/list.                            |
                | `type`              | string | ✅       | Template type (e.g., HTML, Text).                                    |
                | `groupOfUserSend`   | string | ✅       | Newsletter group name to send to when scheduled.                     |
                | `scheduledDate`     | date   | ✅       | YYYY‑MM‑DD date for scheduled send (IST).                            |
                | `scheduledTime`     | string | ✅       | HH:mm time for scheduled send (IST).                                 |
                | `state`             | string | ✅       | Draft / scheduled / sent.                                            |
                | `userGroup`         | string | ✅       | App‑specific user group filter (optional).                           |
                ...
                ```bash
                    {
                        "success" : true,
                        "message" : "Template 'Welcome‑2025‑05' updated in SES and MongoDB.",
                        "template": {
                            "_id"           : "6651ea35e2f2ba1c3c509046",
                            "name"          : "Welcome‑2025‑05",
                            "subject"       : "Updated Welcome Subject",
                ...
                #### 400 Not Found
                ```bash
                { "error": "Template 'Welcome‑2025‑05' not found." }
                ```
                #### 500 Internal Server Error
                ```bash
               ...
               ---
                ...
                | Field          | Type   | Required | Description                                      |
                |----------------|--------|----------|--------------------------------------------------|
                | `templateName` | string | ✅       | Name of the SES template to schedule             |
                | `groupName`    | string | ✅       | Newsletter group name to receive the send        |
                | `date`         | date   | ✅       | YYYY‑MM‑DD date for scheduled send (IST)         |
                | `time`         | string | ✅       | HH:mm time for scheduled send (IST)              |
...
## 6 Dispatch Pipeline (SNS → SQS → Lambda)
...
<Accordion title="📬 Email Notification Flow: SNS → SQS → Lambda → SES with DLQ">
  <Steps>
    <Step title="Overview">
