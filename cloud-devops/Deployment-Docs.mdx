...
description: 'Comprehensive guide for deploying Next.js applications on DigitalOcean Ubuntu Server'
---


# Deployment Checklist for Next.js Applications on DigitalOcean Ubuntu Server

This comprehensive guide ensures thorough preparation, deployment, and monitoring of Next.js applications on a DigitalOcean Ubuntu server.
...
# Deployment Steps for Next.js Applications on DigitalOcean Ubuntu Server
...
<Steps>
  <Step title="Provision the DigitalOcean Droplet">
    <AccordionGroup>
  ...
  <Step title="Point Domain Names to the Server">
    <AccordionGroup>
      <Accordion title="Update DNS Records">
        ...
        - [ ] Create A records for your main domain and any subdomains, pointing them to your DigitalOcean server's IP address.
      </Accordion>
    </AccordionGroup>
  </Step>
  ...
  <Step title="Set Up the Second Next.js Application">
    <AccordionGroup>
      <Accordion title="Create Another Next.js App">
        Repeat steps 7 to 9 for the second Next.js app. Use a different port and directory.
        ```bash
        npx create-next-app@latest testomarl
        cd testomarl
        npm run build
        pm2 start npm --name testomarl -- start
        ```
      </Accordion>
      <Accordion title="Configure Nginx">
        Create another Nginx configuration:
        ```bash
        sudo nano /etc/nginx/sites-available/testomarl
        ```
        Add the configuration:
        ```nginx
        server {
          listen 80;
          server_name test.omarl.com;
      ...
      <Accordion title="Enable the Site">
        Enable the configuration:
        ```bash
        sudo ln -s /etc/nginx/sites-available/testomarl /etc/nginx/sites-enabled/
        sudo systemctl restart nginx
        ```
      </Accordion>
      <Accordion title="Obtain SSL Certificate">
        Obtain an SSL certificate:
        ```bash
        sudo certbot --nginx -d test.omarl.com
        ```
      </Accordion>
    </AccordionGroup>
  </Step>
