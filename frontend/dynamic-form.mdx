...
interface LiveSessionFormProps {
  ...
  onActionButtonClick?: () => void; // ✅ Add this
...
const MODULE_NAME_MAP: Record<string, string> = {
  ...
  // ✅ NEW: anything under /gymManagement/* is Studio Management
...
const MANUAL_OVERRIDES: Array<{ test: RegExp; module: string }> = [
  // ✅ Entire /gymManagement tree → Studio Management
  {
...
/** Call this anywhere (e.g., a small admin “Access Tools” panel) to add a manual override. */
...
const LiveSessionForm: React.FC<LiveSessionFormProps> = ({
  ...
  // 🔑 this is the final gate you asked for
  const canView = !enforceAccess || superAdmin || !!perms.View;

  // 🔔 show Access Denied toast ONCE, AFTER ACL resolves
  ...
  // ⛔ don’t render the form at all while ACL is loading
  if (enforceAccess && accessLoading) {
    return <div className="p-6 text-neutral-300">Loading...</div>;
  }

  // After accessLoading guard, before return’s main body:
