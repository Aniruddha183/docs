---
title: UserNavbar — Footer
description: Global footer component used across the FitnEarn web app
---

## Overview

`Footer` is the **primary global footer component** used across the FitnEarn web application.  
It combines **content rendering**, **newsletter subscription**, **dynamic CMS data**, and **navigation shortcuts** into a single reusable layout component.

This component is **client-side only** and is intentionally stateful.

---

## Responsibilities

The `Footer` component handles:

- Newsletter subscription (email input + API call)
- Success confirmation via modal
- Dynamic footer description fetched from CMS
- Brand identity (logo + tagline)
- Social media links
- Navigation links (services, policies, explore)
- Conditional routing (auth-aware navigation)
...
### 1. NewsletterConfirmationModal

An **internal modal component** shown after a successful newsletter subscription.

**Purpose**

- Provides immediate visual confirmation
- Encourages users to check their email

**Key traits**

- Fully controlled via local state (`success`)
- Overlay-style modal (no external dialog dependency)
- Close button supported

---

### 2. Footer (Main Component)

```ts
const Footer: NextPage<LogoLinksType>;
```
...
| ----------- | ------------------- | ---------------------------------------------- |
| `className` | `string` (optional) | Allows style extension/custom layout overrides |
...
| State                | Purpose                                 |
| -------------------- | --------------------------------------- |
| `email`              | Newsletter input value                  |
| `loading`            | Controls subscribe button loading state |
| `success`            | Triggers confirmation modal             |
| `subscriptionStatus` | Tracks subscription progress            |
| `footerInfo`         | CMS-driven footer description           |
...
- `js-cookie` – token detection
- `axios` – API requests
- `useToast` – error & validation feedback

### Environment Variables

- `NEXT_PUBLIC_BASE_API_ENDPOINT`
- `NEXT_PUBLIC_ADMIN_BASE_API`

---

## Data Fetching (CMS)

Footer description is fetched from admin CMS:

```ts
GET / api / fitnearn / web / admin / contentPublic / CNT00033;
```

Stored under:

```ts
footerInfo.fitnearnFooter.description;
```

This allows **content updates without redeploying frontend**.
...
- Uses regex-based email validation
- Prevents API call if invalid

### API Call

```ts
POST / api / fitnearn / web / users / newsletter - subscribe;
```
...
- Displays backend message when available

---

## Navigation Logic

### Auth-aware Blog Creation

```ts
handleUserToCreateBlog();
```

- If user is logged in (`genToken` exists) → redirect to blog creation
- Else → redirect to signup

This avoids exposing protected flows to anonymous users.
...
- Early bird signup
- Partner as coach / gym / brand
- External Formbricks links
...
- Scroll-safe and overflow-aware
...
Typically rendered via layout utilities (e.g. `ShowFooter`):
...
> This is the complete implementation of the `Footer` component as used in production.
> You can copy–paste this directly if you need the exact behavior.
...
const adminBaseApi = process.env.NEXT_PUBLIC_ADMIN_BASE_API;
  ...
  const footerData = async () => {
    try {
      const response = await fetch(
        `${adminBaseApi}/api/fitnearn/web/admin/contentPublic/CNT00033`
      );
  ...
  const handleSubscribe = async (e: any) => {
    ...
    try {
      ...
      if (isValidEmail(email)) {
        ...
        setLoading(false);
        toast({
          title: "Invalid Email!, Please enter valid Email ID.",
      ...
      toast({
        title: error.response?.data?.message || "Error while subscribing!",
              ...
              <div className="self-stretch flex flex-col items-start justify-start gap-[20px] max-w-full">
                <div className="self-stretch flex flex-row items-start justify-start py-0 pr-0 pl-[5px] box-border max-w-full">
                  <p className="m-0 flex-1 relative leading-[24px] inline-block max-w-full z-[1]">
                    {/* {footerInfo ? footerInfo?.fitnearnFooter?.description : "Welcome to FitnEarn, where fitness meets reward in an innovative and engaging way. Our platform is designed to inspire, motivate, you towards a healthier lifestyle, as you achieve your fitness goals."} */}
                    {footerInfo?.fitnearnFooter?.description}
                  </p>
                </div>
              ...
              <div className="self-stretch mq450:justify-between mq450:py-2  flex flex-row items-start justify-start py-0 px-0 gap-[32px] mq750:flex-wrap mq750:gap-[16px]">
                ...
                <div className="flex flex-col items-start justify-start gap-[16px]">
                  ...
                  <div className="flex flex-col items-start justify-center gap-[12px] text-sm text-neutral-300">
                    ...
                    <Link href="/workout?LiveSession=true">
                      <div className="rounded flex flex-row items-center justify-start py-[3.5px] px-0 gap-[8px]">
...
- ❌ Do not move newsletter logic outside Footer
- ❌ Do not convert to server component (uses cookies, router, hooks)
- ❌ Do not hardcode CMS text content

---

## Why This Component Is Large (Intentionally)

- Footer is **business-critical**
- Centralizes all bottom-navigation & compliance links
- Reduces duplication across pages
- Designed to evolve with CMS-driven content

---

## Summary

The `Footer` component is a **feature-rich, stateful global layout component** that:

- Encourages user engagement
- Handles subscriptions safely
- Adapts via CMS
- Respects auth boundaries
- Anchors the product’s brand identity

This is **not a simple UI footer** — it is a functional product surface and should be treated as such.
