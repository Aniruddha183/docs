...
- **Column Search**: Each column includes an independent search input.
- **Combined Search Query**: All active column searches are merged into a single query string.
- **Sortable Columns**: Click column headers to toggle ascending/descending sort.
- **Pagination**: Paginated data with ellipses for large page sets.
- **Row Selection**: Select individual rows or all rows on the current page.
- **Preselected Rows**: Initialize selected rows via `preselectedItems`.
- **Expandable Rows**: Parent rows can expand to reveal child rows.
- **Toggle Column**: Optional boolean toggle per row (e.g. attendance, status).
- **Permission-Safe Selection**: Row checkboxes can be conditionally disabled.
- **Custom Cell Styling**: Apply conditional styles per column.
- **Cell Interaction**: Capture clicks on individual cells.
...
| `onCellClick`           | `(item: any, columnKey: string) => void`                                                             | `undefined`    | Called when a table cell is clicked.                                      |
| `isRowCheckboxDisabled` | `(row: any) => boolean`                                                                              | `undefined`    | Disables row selection checkbox conditionally.                            |
| `showToggleColumn`      | `boolean`                                                                                            | `false`        | Shows a boolean toggle column per row.                                    |
| `toggleKey`             | `string`                                                                                             | `"isAttended"` | Data key used to read initial toggle state.                               |
| `onToggleChange`        | `(item: any, value: boolean) => void`                                                                | `undefined`    | Callback when toggle value changes.                                       |

#### Data & State Behavior

- Data is fetched using the `useData(page, search)` function.
- Search values from all columns are combined into a single query string.
- Sorting is applied client-side before rendering.
- `preselectedItems` are applied only once to avoid infinite loops.
- Row selection state is maintained internally and synced via callbacks.

#### Selection & Toggle Lifecycle

- Selecting a row updates internal selection state.
- Selecting all rows only affects the current page.
- Disabled rows cannot be deselected.
- Toggle states are synced from incoming data when data changes.
- Toggle clicks update local state immediately and notify the parent.
...
2. **Table Body**:
   ...
   - Renders sorted rows for the current page.
   - Supports expandable parent and child rows.
   - Allows conditional cell rendering and styling.
   - Handles row selection, toggles, and details actions.

3. **Pagination**:
   - Supports previous/next navigation.
   - Uses ellipses when total pages exceed visible limits.
   - Pagination operates on server-provided page counts.

#### Behavioral Notes

- Sorting is client-side and does not re-fetch data.
- Search resets pagination context implicitly.
- Toggle state is always coerced to a boolean.
- Expandable rows share the same selection logic as parent rows.
- Details actions are optional and safe to omit.
...
- Do not mutate `data` directly inside the table.
- Always extend behavior via props, not internal state.
- Keep `uniqueKey` stable across parent and child rows.
- Avoid adding new local pagination assumptions.
- Gate new interactive columns behind explicit props.
- Preserve preselection safeguards to prevent re-render loops.
