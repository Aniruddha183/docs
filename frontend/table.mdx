...
| `useData`           | `(page: number, search: string) => { data: any[]; totalPages: number; preselectedItems?: string[]; }` | `undefined` | Function to fetch table data dynamically based on page and search queries. |
