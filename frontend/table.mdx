...
The `DynamicTable` component is a versatile and customizable table that supports pagination, search, row selection, sorting, and conditional styling. It is designed to handle large datasets efficiently with a dynamic data-fetching mechanism.
...
| `sortOrder`         | `'asc' \| 'desc'`                                                                                    | `'desc'`    | Sorting order for rows.                                             |
| `preselectedItems`  | `string[]`                                                                                           | `[]`        | Optional initial list of selected row IDs, returned from `useData`. |
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        {/* ✅ SCROLLABLE BODY TABLE (keeps alignment with header) */}
        <div
          ...
          <Table className="table-fixed w-full">
            <TableBody>
              {sortedData.map((item, rowIndex) => (
                <TableRow key={`${item[uniqueKey]}-${rowIndex}`}>
                  <TableCell className="text-center w-[48px]">
                    <Checkbox
