...
The `DynamicTable` component is a versatile and customizable table that supports pagination, search, and row selection. It is designed to handle large datasets efficiently with a dynamic data-fetching mechanism.

#### Features

- **Searchable Columns**: Each column includes a search field for filtering data.
- **Pagination**: Easily navigate between multiple pages of data.
- **Row Selection**: Supports selecting and deselecting rows for further actions.
...
const ExampleComponent = () => {
  ...
  const useData = (page, search) => {
    ...
    return {
      data: [
        { sessionId: "1", name: "John Doe", email: "john@example.com", role: "Admin" },
        { sessionId: "2", name: "Jane Doe", email: "jane@example.com", role: "User" },
      ],
      totalPages: 2,
    };
  };
...
3. **Pagination**:
   ...
   - Supports dynamic page ranges and ellipses for large datasets.
...
export default function DynamicTable({
  ...
  return (
    <div className={`p-4 rounded-xl ${showBorder ? "border border-neutral-700" : ""}`}>
      <Table>
        {/* Table Header */}
        {/* Table Body */}
        {/* Pagination */}
